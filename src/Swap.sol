...
contract SocksSwap is ERC1155TokenReceiver {
    ...
    mapping(address => mapping(Sock => uint)) balanceOf;
    ...
    function inflate(Sock expected) public {
        ...
        require(block.difficulty % 2 == uint(expected));

        for (uint i = 0; i < BOOTSTRAP;) {
            socks.mint();
            unchecked {
                ++i;
            }
    ...
    function swap(Sock from, Sock to) external {
        socks.safeTransferFrom(msg.sender, address(this), uint(from), 1, "");
    ...
    function withdraw() external {
        (uint balLeft, uint balRight) = (balanceOf[msg.sender][Sock.Left], balanceOf[msg.sender][Sock.Right]);
        balanceOf[msg.sender][Sock.Left] = 0;
        balanceOf[msg.sender][Sock.Right] = 0;
        if (balLeft > 0) {
            socks.safeTransferFrom(address(this), msg.sender, uint(Sock.Left), balLeft, "");
        }
        if (balRight > 0) {
            socks.safeTransferFrom(address(this), msg.sender, uint(Sock.Right), balRight, "");
        }
    }
}
